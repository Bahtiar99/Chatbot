...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = "Bahtiar",
            BATCH_ACADEMY = "Phoenix Day",
            GITHUB_REPO_LINK = "https://github.com/Bahtiar99/Chatbot.git"
        )
    ...
    if selected_campaign_id in unique_campaign:
        ...
        with open('template_text/summary.txt', mode='r', encoding='utf-8') as f:
            ...
            summary = temp.substitute(
                ...
                END_DATE = end_date,
                TOTAL_SPENT = f"Value of cost is {total_spent:,.0f} USD.",
                TOTAL_CONVERSION = f"Value of cost is {total_conversion:,.0f} USD.",
                CPC = f"Value of cost is {cpc:,.1f} USD."
            )
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        df_campaign = df[df['campaign_id']==selected_campaign_id]
        df_plot = df_campaign.groupby('age')[['spent','approved_conversion']].sum().reset_index()
